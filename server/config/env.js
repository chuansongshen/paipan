...
const envSchema = z.object({
  ...
  WECHAT_APP_SECRET: z.string().trim().default(''),
  WECHAT_MCH_ID: z.string().trim().default(''),
  WECHAT_NOTIFY_URL: z.string().trim().default(''),
  WECHAT_OAUTH_REDIRECT_URI: z.string().trim().default(''),
  WECHAT_OAUTH_SCOPE: z.string().trim().default('snsapi_base'),
...
export function readEnv(source = process.env) {
  ...
  return {
    ...
    wechatOauthRedirectUri: parsed.data.WECHAT_OAUTH_REDIRECT_URI,
    wechatOauthScope: parsed.data.WECHAT_OAUTH_SCOPE,
